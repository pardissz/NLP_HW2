...
def extract_name(text):
    pattern =r"(انجام دهنده|مسئول|افراد مسئول|مسئولیت).*?(کار|تسک|ددلاین)?.*?(منتقل شد|انتقال یافت|عوض شد|تغییر کرد)?.*?به (.*?)(منتقل شد|انتقال یافت|عوض شد|تغییر کرد)"
    match = re.search(pattern, text)
    if match:
        return match.group(4).strip()
    else:
...
def extract_ending(text):
    pattern =r"(پایان|ددلاین|مهلت|اتمام).*?(کار|تسک)?.*?(منتقل شد|انتقال یافت|عوض شد|تغییر کرد)?.*?به (.*?)(اضافه شد|منتقل شد|انتقال یافت|عوض شد|تغییر کرد)"
    match = re.search(pattern, text)
    if match:
        return match.group(4).strip()
    else:
