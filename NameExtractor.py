...
"""
text_boys_500 = """
اَرَشک	پسر و جانشین اردشیر سومبرگشت به لیست
...
بَکتاش	خوش اخلاق و با صفات نیکوبرگشت به لیست
...
ثامر	میوه دهنده، ثمردهندهبرگشت به لیست
ثاقب	(عربی) هوشیار، تند دریابنده (ذهن، فکر )، روشن، فروزان
ثامن	هشتم، هشتمین
جاويد	هميشگی، ابدیبرگشت به لیست
...
دانا	دارنده‌ی دانش و آگاهی؛ آگاهبرگشت به لیست
...
دومان	مه، غبار
رادین	حقیقی، واقعی، راست قامتبرگشت به لیست
...
زامیاد	یار دانا و دانشمندبرگشت به لیست
...
شَنتیا	نام حضرت علی (ع) در زبور داوود، پیروزبرگشت به لیست
...
صابر	صبور، صبر کننده، شکیبا؛ از نامهای خداوندبرگشت به لیست
...
طغرل	پرنده شکاری از خانواده بازبرگشت به لیست
...
ظافر	ظفریابنده، پیروزی یابندهبرگشت به لیست
ظهیر	پشتیبان، یاور

عادل	با انصافبرگشت به لیست
...
غَدیر	آبگیری است بین مکه و مدینهبرگشت به لیست
...
کارن	اسم پسرانه به معنی شجاع و دلیر، نام فرزند کاوه آهنگربرگشت به لیست
...
مَهدیار	محافظ و نگهبانِ سرزمین و میهنبرگشت به لیست
مَهیار	ماهیار
مِهداد	بزرگزاده
...
نادین	در جریان، پویا و پرتحرک، الهه رودخانه، جاریبرگشت به لیست
نامور	نام آور، مشهور
...
هابیل	معرب از عبری، نفس یا بخار، نام پسر آدم (ع)برگشت به لیست
...
یارتا	همتای یار، همچون یاربرگشت به لیست
...
بانو	خانم، کلمه احترام درباره بانوانبرگشت به لیست
...
بینا 	روشن، دل آگاه
پَرَند 	پروین، حریر ساده؛ گروهی از گیاهانبرگشت به لیست
...
ثَمینا 	(ثمین + الف نسبت) منسوب به ثمین، ثمینبرگشت به لیست
...
جَمیله 	مؤنث جمیل، جمیل؛ زیبابرگشت به لیست
...
حَلیا 	زیور و آرایشبرگشت به لیست
...
خرمن	توده غله درو شده، توده و مقدار انبوه از هر چیزبرگشت به لیست
...
دَستنبو	میوه ای خوشبوبرگشت به لیست
...
رَسابانو 	بانوی برگزیده و بلند بالابرگشت به لیست
...
زُمرد	گوهر، از سنگ‌های قیمتیبرگشت به لیست
...
سَرو	از انواع درختان که همیشه سبز است؛ (به مجاز) شاداب و با طراوتبرگشت به لیست
...
شِیدا 	عاشق، دلداده؛ آشفته و پریشانبرگشت به لیست
...
صَبا 	پیام رسان میان عاشق و معشوقبرگشت به لیست
...
طَراوت 	تر و تازگی؛ (به مجاز) سامان و رونقبرگشت به لیست
...
فاتن 	فتنه انگیز، آنکه آشوب بپا می‌کندبرگشت به لیست
...
کُنیا 	دختری که ازدواج نکرده استبرگشت به لیست
...
لَچَک 	دستمال سه گوشبرگشت به لیست
...
مَهدیس	مانند ماه؛ (به مجاز) زیباروبرگشت به لیست
...
نادیا 	زن خوش آوتزبرگشت به لیست
ناربن 	 درخت انار
...
هُووی 	نام زن اشوزرتشت, دختر فرشوشتربرگشت به لیست
...
یَسنا 	پرستش، از بخش‌های اوستابرگشت به لیست
...
lines_men = text_boys_500.strip().split("\n")
lines_women = text_girls_2000.strip().split("\n")
